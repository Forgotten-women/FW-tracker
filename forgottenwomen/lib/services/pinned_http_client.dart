// TLS trust-anchor restriction.
//
// The API/OTA/notification clients previously used the platform's full
// system trust store, meaning any of the 100+ CAs it trusts -- including
// one added by an MDM profile or a network-injected root on a compromised
// Wi-Fi -- could vouch for a man-in-the-middle and silently intercept
...
http.Client createPinnedHttpClient() {
  try {
    final context = SecurityContext(withTrustedRoots: false)
      ..setTrustedCertificatesBytes(utf8.encode(_trustedRootPem));
    return IOClient(HttpClient(context: context));
  } catch (_) {
    // If the platform can't build a restricted SecurityContext, fall back
    // to the default client (full system trust store) rather than leaving
    // the app unable to make any request at all.
    return http.Client();
  }
}
