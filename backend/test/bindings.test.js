...
function arpSighting(mac, ip, atMs) {
  return P.recordEvent({ employeeId: null, source: 'ARP', mac, srcIp: ip, observedAt: atMs });
}
/** What the ESP sniffer does: a MAC with no IP at all. */
function espSighting(mac, atMs) {
  return P.recordEvent({ employeeId: null, source: 'ESP_SNIFFER', mac, observedAt: atMs });
}
/** An authenticated app heartbeat. */
function appPing(employeeId, deviceId, atMs) {
...
test('an authenticated ping binds the phone address seen on the same IP', () => {
  const emp = makeEmployee('emp_bind');
  const dev = makeDevice('dev_bind', emp);
  const now = T.wallClockToEpoch(T.dateKey(), '11:00');

  arpSighting(PHONE_MAC, OFFICE_IP, now);
  appPing(emp, dev, now);

  const result = bindings.bindFromAuthenticatedPing({
  ...
  const resolved = bindings.employeeForMac(T.hashMac(PHONE_MAC, MAC_SALT), now);
  assert.equal(resolved.employeeId, emp);
});

test('an unbound address stays anonymous', () => {
  const resolved = bindings.employeeForMac(T.hashMac('99:99:99:99:99:99', MAC_SALT));
  assert.equal(resolved, null, 'a MAC nobody proved must never be attributed');
});

test('binding is refused when two addresses share one IP', () => {
  const emp = makeEmployee('emp_ambig');
  const dev = makeDevice('dev_ambig', emp);
  ...
  arpSighting('11:11:11:11:11:11', ip, now - 30 * 1000);
  arpSighting('22:22:22:22:22:22', ip, now - 10 * 1000);
...
test('binding is refused when nothing was seen on that IP recently', () => {
  const emp = makeEmployee('emp_nosight');
  const result = bindings.bindFromAuthenticatedPing({
...
test('presence continues after the app is closed', () => {
  const emp = makeEmployee('emp_closed');
  const dev = makeDevice('dev_closed', emp);
  ...
  arpSighting(mac, ip, arrive);
  appPing(emp, dev, arrive);
  bindings.bindFromAuthenticatedPing({ employeeId: emp, deviceId: dev, srcIp: ip, nowMs: arrive });

  // 11:05 - closes the app. No further heartbeats, ever.
  const appClosed = arrive + 5 * MIN;
  appPing(emp, dev, appClosed);

  // 11:05 to 15:00 - only the ESP, which cannot see the app at all.
  for (let m = 10; m <= 240; m += 10) {
    espSighting(mac, arrive + m * MIN);
  }
  ...
  const day = P.deriveDay(emp, T.dateKey(arrive), now);
  ...
  const quiet = P.deriveDay(emp, T.dateKey(arrive), arrive + 250 * MIN);
  assert.equal(quiet.status, 'GRACE_PERIOD', '10 minutes after the last sighting');
  const gone = P.deriveDay(emp, T.dateKey(arrive), arrive + 300 * MIN);
  assert.equal(gone.status, 'AWAY', 'an hour after the last sighting');
});

test('leaving the office does end presence', () => {
  const emp = makeEmployee('emp_left');
  const dev = makeDevice('dev_left', emp);
  ...
  for (let m = 0; m <= 120; m += 10) espSighting(mac, arrive + m * MIN);
...
test('a sensor sighting of a bound address is recorded against that employee', () => {
  const emp = makeEmployee('emp_attr');
  const dev = makeDevice('dev_attr', emp);
  const mac = 'de:ad:be:ef:00:03';
  const ip = '192.168.18.62';
  const now = T.wallClockToEpoch(T.dateKey(), '11:00');

  arpSighting(mac, ip, now);
  bindings.bindFromAuthenticatedPing({ employeeId: emp, deviceId: dev, srcIp: ip, nowMs: now });
...
test('an expired binding stops attributing', () => {
  const emp = makeEmployee('emp_expire');
  const dev = makeDevice('dev_expire', emp);
  ...
  assert.ok(bindings.employeeForMac(macHash, now));

  // Well past the TTL, with the app never reopened.
  const later = now + bindings.BINDING_TTL_MS + MIN;
  assert.equal(bindings.employeeForMac(macHash, later), null,
    'a binding must not follow a rotated or reassigned address forever');

  bindings.expireStale(later);
  const row = db.prepare('SELECT revoked_reason FROM device_mac_bindings WHERE mac_hash = ?').get(macHash);
  assert.match(row.revoked_reason, /Expired/);
});

test('revoking a device revokes its binding', () => {
  const emp = makeEmployee('emp_revoke');
  const dev = makeDevice('dev_revoke', emp);
  ...
  arpSighting(mac, ip, now);
  bindings.bindFromAuthenticatedPing({ employeeId: emp, deviceId: dev, srcIp: ip, nowMs: now });
  assert.ok(bindings.employeeForMac(T.hashMac(mac, MAC_SALT), now));

  bindings.revokeForDevice(dev, 'Device revoked by administrator');

  assert.equal(bindings.employeeForMac(T.hashMac(mac, MAC_SALT), now), null,
    'a handed-back phone must not keep clocking in its old owner');
});

test('a newer authenticated proof takes the address from the older one', () => {
  const first = makeEmployee('emp_first', 'First Owner');
  const second = makeEmployee('emp_second', 'Second Owner');
  const devA = makeDevice('dev_first', first);
  const devB = makeDevice('dev_second', second);
  const mac = 'de:ad:be:ef:00:06';
  const ip = '192.168.18.65';
  const now = T.now();

  arpSighting(mac, ip, now);
  bindings.bindFromAuthenticatedPing({ employeeId: first, deviceId: devA, srcIp: ip, nowMs: now });
  assert.equal(bindings.employeeForMac(T.hashMac(mac, MAC_SALT), now).employeeId, first);

  // The address is reassigned and a different employee proves it.
  const later = now + 10 * MIN;
  arpSighting(mac, ip, later);
  bindings.bindFromAuthenticatedPing({ employeeId: second, deviceId: devB, srcIp: ip, nowMs: later });

  assert.equal(bindings.employeeForMac(T.hashMac(mac, MAC_SALT), later).employeeId, second,
...
test('every binding change is recorded', () => {
  const types = db.prepare('SELECT DISTINCT event_type FROM mac_binding_events').all().map(r => r.event_type);
