...
async function ping(employeeId, deviceId, atMs) {
...
test('no events yields NOT_CHECKED_IN', async () => {
  ...
  const d = await P.deriveDay(emp, T.dateKey(), T.now());
  assert.equal(d.status, 'NOT_CHECKED_IN');
  assert.equal(d.totalMinutes, 0);
  assert.deepEqual(d.sessions, []);
});

test('continuous pings form one open session and count minutes', async () => {
  const emp = await makeEmployee('e_cont');
  const dev = await makeDevice('d_cont', emp);
  const now = T.wallClockToEpoch(T.dateKey(), '12:00');
  // 09:00 -> 12:00, one ping a minute apart is far inside the grace window.
  for (let m = 0; m <= 180; m += 5) await ping(emp, dev, now - (180 - m) * MIN);

  const d = await P.deriveDay(emp, T.dateKey(now), now);
...
test('a gap longer than the grace period splits the session', async () => {
  const emp = await makeEmployee('e_gap');
  const dev = await makeDevice('d_gap', emp);
  const base = T.wallClockToEpoch(T.dateKey(), '09:00');
  // 09:00-10:00 present, then a 90 minute absence, then 11:30-12:00 present.
  for (let m = 0; m <= 60; m += 5) await ping(emp, dev, base + m * MIN);
  for (let m = 150; m <= 180; m += 5) await ping(emp, dev, base + m * MIN);

  const now = base + 180 * MIN;
  const d = await P.deriveDay(emp, T.dateKey(base), now);
...
test('three-state model: IN_OFFICE -> GRACE_PERIOD -> AWAY', async () => {
  const emp = await makeEmployee('e_states');
  const dev = await makeDevice('d_states', emp);
  const base = T.wallClockToEpoch(T.dateKey(), '09:00');
  await ping(emp, dev, base);

  // activeThreshold = 3m, gracePeriod = 15m
  assert.equal(await (await P.deriveDay(emp, T.dateKey(base), base + 1 * MIN)).status, 'IN_OFFICE');
  assert.equal(await (await P.deriveDay(emp, T.dateKey(base), base + 5 * MIN)).status, 'GRACE_PERIOD');
  assert.equal(await (await P.deriveDay(emp, T.dateKey(base), base + 40 * MIN)).status, 'AWAY');
});
...
test('a day in the past is closed and its minutes do not grow', async () => {
  const emp = await makeEmployee('e_rollover');
  const dev = await makeDevice('d_rollover', emp);
  ...
  for (let m = 0; m <= 480; m += 10) await ping(emp, dev, start + m * MIN);

  const noonToday = T.wallClockToEpoch(today, '12:00');
  const midnightTonight = T.wallClockToEpoch(today, '23:59');

  const atNoon = await P.deriveDay(emp, yesterday, noonToday);
  const atMidnight = await P.deriveDay(emp, yesterday, midnightTonight);
...
test('events cannot leak across the midnight boundary', async () => {
  const emp = await makeEmployee('e_boundary');
  const dev = await makeDevice('d_boundary', emp);
  const today = T.dateKey();
  const yesterday = T.dateKey(T.startOfDay(today) - 12 * 60 * MIN);

  await ping(emp, dev, T.wallClockToEpoch(yesterday, '23:50'));
  await ping(emp, dev, T.wallClockToEpoch(today, '00:10'));

  const y = await P.deriveDay(emp, yesterday, T.wallClockToEpoch(today, '12:00'));
  const t = await P.deriveDay(emp, today, T.wallClockToEpoch(today, '12:00'));
...
test('replaying a buffered heartbeat is idempotent', async () => {
  const emp = await makeEmployee('e_dedupe');
  const dev = await makeDevice('d_dedupe', emp);
  const at = T.wallClockToEpoch(T.dateKey(), '10:00');

  const first = await ping(emp, dev, at);
  const replay = await ping(emp, dev, at);
...
test('a heartbeat from outside the office does not count as attendance', async () => {
  const emp = await makeEmployee('e_remote');
  const dev = await makeDevice('d_remote', emp);
  const at = T.wallClockToEpoch(T.dateKey(), '10:00');
  ...
  const d = await P.deriveDay(emp, T.dateKey(at), at + MIN);
...
test('the cached attendance row matches a fresh derivation', async () => {
  const emp = await makeEmployee('e_cache');
  const dev = await makeDevice('d_cache', emp);
  const base = T.wallClockToEpoch(T.dateKey(), '09:00');
  for (let m = 0; m <= 60; m += 5) await ping(emp, dev, base + m * MIN);

  const now = base + 61 * MIN;
  const derived = await P.recomputeDay(emp, T.dateKey(base), now);
